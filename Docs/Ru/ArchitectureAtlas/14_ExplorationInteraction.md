...
Exploration Interaction отвечает за обнаружение доступных целей рядом с игроком, выбор одной текущей цели и запуск ее взаимодействия по вводу игрока.

Система сочетает:

- радиус вокруг центра тела игрока;
- последнее ненулевое направление движения, приведенное к одному из восьми направлений;
...
На `InteractionOrigin` нет Collider2D и trigger-логики. Он не создает физические контакты.

Мировая позиция origin вычисляется через физическую позицию `Rigidbody2D`. Благодаря этому область поиска не отстает от игрока при включенной Rigidbody2D interpolation.

## Player Collider

Физический `Collider2D` игрока остается у ног и отвечает только за столкновения с миром.
...
Направление до цели остается точным и вычисляется по вектору от `InteractionOrigin` до ближайшей точки collider. Оно не округляется до восьми направлений.
...
Границы `±22.5°` относятся к priority `0`. Границы `±45°` еще входят в допустимую область.
...
1. Collider найден через `Physics2D.OverlapCircle` на разрешенном interaction-layer.
...
Кандидаты сравниваются в строго определенном порядке.
...
| `ScanIntervalSeconds` | `0.05` | Частота пересчета текущей цели |
...
`CurrentTargetChanged` публикуется только при изменении цели или ее исходного collider.
...
Prompt и highlight подписываются на выбранную цель, но не ищут и не выбирают ее.
...
Центром поиска является отдельный `InteractionOrigin`. Collider у ног остается частью физики.
...
## ❌ Выбирать ближайшую цель до учета direction priority
