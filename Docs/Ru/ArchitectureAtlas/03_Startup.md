...
Startup инкапсулирует всю логику выбора первой Main Phase и полностью отделяет её от процесса запуска приложения.
...
После выбора стартовой фазы управление передаётся GameStateMachine.
